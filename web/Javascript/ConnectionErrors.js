...
function SetErrorMode(Mode)
{
	...
	if (ErrorMode == LANErrorMode)
		...
		if (TVShell.ConnectionManager.EthernetAdapter.Enabled || !TVShell.ConnectionManager.WirelessAdapter) {
			ErrorAdapter = TVShell.ConnectionManager.EthernetAdapter;
		} else {
			ErrorAdapter = TVShell.ConnectionManager.WirelessAdapter;
		}
	} else {
	...
	if (!UsingBroadband)
	{
		switch (ErrorCode)
		{
			...
			case ConnectError_AuthFailure:
				...
				break;
			case ConnectError_DialNextNumber:
				output = "Phone number failed, trying next phone number";
				break;
			default:
	...
	if (!UsingBroadband) {
		switch (ErrorCode) {
			...
			case ConnectError_LineReversal:
				...
				break;
			case ConnectError_LineInUse:
				output = "<P>Your "+ProductShortName+" was unable to connect to "+NetworkShortName+" because another phone on the same line was in use.</P>" +
					MultipleStepText + 
					"<LI><SPAN class=li>Make sure none of the phones on this line are currently in use or off the hook.</SPAN> \
					<LI><SPAN class=li>If you are certain that the line was not in use try selecting <EM>Don't wait for dial tone before connecting</EM> in <EM>Settings</EM>. </SPAN> \
					</UL>";
				break;
			case ConnectError_NoDialTone:
				output = "<P>Your "+ProductShortName+" could not connect to the "+NetworkFullName+" because it did not detect a dial tone on your phone line.  " +
					MultipleStepText +
					SuggestLiveLineTestText + 
					"<LI><SPAN class=li>If you have voice mail service on your phone line, select <EM>Don't wait for dial tone</EM> in <EM>Settings</EM>. </SPAN>\
					</UL>";
				break;
			case ConnectError_Busy:
...
function ShortErrorMessage()
{
	...
	switch (ErrorCode)
	{
		...
		case ConnectError_AssociationFailed:
			...
			break;
		case ConnectError_DialNextNumber:
			errmsg = "Failed to connect to the service with this phone number, trying next phone number.";
			break;
		case ConnectError_SlowConnectionRedial:
			errmsg = "Slow dial-up connection, trying next selected telephone number.";
			break;
		default:
