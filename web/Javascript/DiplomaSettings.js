...
var MDIPLOMA_OPW	= 0x00000004;
var MDIPLOMA_SAVEALBUM = 0x00000008;
...
function DiplomaCustomMessageBox( noDialogReturn, diploma, text, title, buttons )
{
	...
	if (IsRegistered()) 
	{
		...
		if ( (old & diploma) == 0 ){
			var retVal;
			retVal = TVShell.PanelManager.CustomMessageBox(text,title,buttons,defaultButton,errorString,trusted,iconType,size);
			if ( retVal == noDialogReturn ){				
				TVShell.UserManager.CurrentDiplomaSettings = old | diploma;
				TVShell.UserManager.Save();
			}
			noDialogReturn = retVal;
		}
